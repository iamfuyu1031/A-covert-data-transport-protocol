...
def convert_str_to_binary(input_str):
	binary = bin(int(binascii.hexlify(input_str),16))
	return binary[2:].zfill(len(input_str)*8)
...
def map_list_to_bit(my_list):
	...
	#folder = 'pickle_output'
	#all_combos = pickle.load(open(working_dir + '/' + folder + '/' + str(integer),'rb'))
...
def hmm_stop_at_space(message, filename):
	...
	space = 0
	while message != '':
		last_message = full_message[:len(message)*(-1)]
		flag = 0
		j = j + 1
		choice, end, bit_list = read_hmm(filename, start, all_prob, all_state)
		for i in range(len(bit_list)):
			one_length = len(bit_list[i])
			try:
				ind = bit_list.index(message[:one_length])
				flag = 1
				break
			except:
				continue
		if flag == 1:
			start = end[ind]
			message = message[one_length:]
			output += choice[ind]
		# For the last message
		else:
			for i in range(len(bit_list)):
				if bit_list[i][:len(message)] == message and choice[i] != ' ':
					output += (choice[i] + str(len(bit_list[i]) - len(message)).zfill(2))
					break
			message = ''
		if ' ' in output:
			space = 1
			break
	if space == 1:
		final_output = int(math.floor(len(last_message)/8))
	else:
		final_output = int(math.floor(len(all_message)/8))

	return final_output

	

def full_hmm(message, filename):
	j = 0
	output = ''
	# Read the prob file
	prob_file = 'hmm-stat-prob-new'
...
def add_random_prefix(message):
	symbol = 'abcdefghijklmnopqrstuvwxyz0123456789'
	num = random.randint(0, len(symbol)-1)
	message = symbol[num] + message
	return message

def add_prefix(message, symbol):
	#shuffled_symbol = ''.join(random.sample(symbol,len(symbol)))
	random.shuffle(symbol)
	tmp = symbol[0]
	del symbol[0]
	return tmp + message, symbol


def add_2_prefix(message, symbol):
	shuffled_symbol = ''.join(random.sample(symbol,len(symbol)))
	return shuffled_symbol[0] + shuffled_symbol[1] + message, shuffled_symbol[2:]

def add_prefix_test(message, symbol):
	random.shuffle(symbol)
	tmp = '5'
	return tmp + message, symbol
...
if __name__ == '__main__':
	...
	input_str = 'hello'
	filename = '500KL3.fsa'
	full_symbol = ['e', 'g', 'r', 's', 't', 'u', 'v', 'y', 'z', '1', '2', '3', '4', '5', '6', '7', '8', '9']
	prefix = 1
	# AES part
	block_size =  16
	key_file = 'aes_key'
	...
	while remaining_message != '' :
		...
		remaining_message = convert_str_to_binary(input_str)
		# wait until there are at least 2 charaters that will give non-space output
		while len(finished_message) <= (prefix+1) * 8:
			num = hmm_stop_at_space(remaining_message, filename)
			finished_message = remaining_message[:num * 8]
			if len(finished_message) >= (prefix+1) * 8:
				break
			# If there is not, remove the first character from the input string, and put another one
			else:
				# Remove the original prefix
				input_str = input_str[prefix:]
				# Add a new prefix
				input_str, symbol = add_prefix(input_str, symbol)
				# If the symbol list is empty, refill it
				if symbol == '':
					symbol = full_symbol
			remaining_message = convert_str_to_binary(input_str)
		# try if finished_message + index will generate non-space output
		## If it generates space, index += 1
		## Else: do full_hmm on finished_message + index
		one_str = finished_message + convert_str_to_binary(str(index).zfill(2))
		num2 = hmm_stop_at_space(one_str, filename)
		while num2*8 != len(one_str):
			index += 1
			one_str  = finished_message + convert_str_to_binary(str(index).zfill(2))
			num2 = hmm_stop_at_space(one_str, filename)
		output =  full_hmm(one_str, filename)
		remaining_message = remaining_message[num*8:]
